#!/bin/bash -l
# SLURM submission script template for ft_gpt_headless.py.
# Submit from the repository root with: sbatch scripts/ft_gpt_headless.sh
# Fill in the placeholder <...> values and adjust #SBATCH directives /
# module versions to match your cluster before running.
#SBATCH -J ft_gpt_headless
...
srun python scripts/ft_gpt_headless.py \
    --ckpt_path <headless_model>.ckpt \
